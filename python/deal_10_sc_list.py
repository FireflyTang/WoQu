...
class deal_10_sc_list(dealctrl):
    ...
    def run(self):
        ...
        timelimit=''
        if(time):
            timelimit=('AND sc_time<=%d' % (time))
            
        sql=('''SELECT * FROM 
(SELECT * FROM `member_sc`  WHERE  sc_userid=%d %s LIMIT 10) AS a
LEFT JOIN 
`activity_list`  AS b
ON a.sc_aid=b.aid
LEFT JOIN
...
LEFT JOIN 
(SELECT * FROM `member_sc` WHERE pb_userid=%d) AS d
ON a.sc_aid=d.pb_aid
LEFT JOIN 
(SELECT * FROM `activity_dz` WHERE dz_userid=%d) AS e
ON a.sc_userid=e.dz_userid
LEFT JOIN 
(SELECT * FROM `activity_apply` WHERE userid=%d) as f
ON a.sc_aid=f.userid
